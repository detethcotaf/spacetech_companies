...
```
1. 取引データが暗号化されている
2. トランザクションデータがブロックチェーン上に記録されている
    トランザクションデータをブロックという単位で保持し、それをチェーンでつないでいる。
3. データ改ざんが難しい
    電子署名とハッシュを利用しているため、改竄を容易に検知できる。
4. 分散的に運用されている
    あらゆるユーザーのコンピューターに情報が分散して保存される。一元管理されていないのでシステムが実質的にダウンしない。
    GAFAのように中央集権的なシステムではない。
    ネットワーク上の複数のコンピュータがトランザクションデータを確認・合意し、データも複数で分散保持するため、
    高可用性と同一性を実現している。
```
...
ブロックチェーンネットワークの中で、正しい取引を合意するためのアルゴリズムをコンセンサスアルゴリズムとよぶ。
